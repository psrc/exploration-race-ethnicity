...
# Variables
excel_file_path <- 'household-count/data' #folder path
indicator_file <- 'household-count-by-re.xlsx' # Excel file name

# Order for visuals ----
county_order <- c('Region', 'King', 'Kitsap', 'Pierce', 'Snohomish')
race_order_detail_mp <- c(11, #'Total',
...
race_order_dichot_mp <- c(9, #'Total',
...
race_order_single_mp <- c(8, #'Total',
...
race_order_sp <- c(7, #'Total',
                   10, #'Total People of Color',
                   13, #'White alone',
                   11, #'Total Single-race',
                   8, #'Total Multirace', MAY NEED ADJUSTING WITH NEW NAMES
                   1, #'American Indian or Alaskan Native alone',
                   2, #'Asian alone',
                   3, #'Black or African American alone',
                   4, #'Hispanic or Latino',
                   ...
                   9 #'Total Multirace' MAY NEED ADJUSTING WITH NEW NAMES
)


# ---- WORKSPACE ----
# open .xlsx file
df <- read_excel('C:/Users/mrichards/Documents/GitHub/exploration-race-ethnicity/visuals/household-count-by-re.xlsx',
                 sheet = 2)
df <- read_excel('C:/Users/mrichards/Documents/GitHub/exploration-race-ethnicity/visuals/household-count-by-re-hhsize.xlsx', 
                 sheet = 1)

df_white <- df %>% 
  filter(RACE == "White alone") %>% #replicate row with white so that it can be visualized twice
  rbind(df) #bind to original data 

df_multirace <- df %>%   
  filter(RACE=="Total Multirace") %>% #replicate row with total multirace so that it can be visualized twice
  rbind(df_white)
...
if(any(df_all$TABLE_TYPE == "dichot" & (df_all$HH_SIZE == "single-person"))) {
  df_final <- df_all %>%
    arrange(match(COUNTY, county_order),
            match(ID, race_order_sp))
} else if(any(df_all$TABLE_TYPE != "detail")) {
  print("Not a detail table")}

# Organize data based on desired order
df_final <- df_all %>%
  arrange(match(COUNTY, county_order),
          match(ID, race_order_sp))

# Rename Total row for visuals
df_final$RACE[df_final$RACE=='Total'] <- 'Grand Total'
...
try({
  ...
  # Loop through all processed sheets
  lapply(names(processed_sheets), function(name) {
    cat("\nSheet:", name, "\n")
    print(processed_sheets[[name]])
  })
})
